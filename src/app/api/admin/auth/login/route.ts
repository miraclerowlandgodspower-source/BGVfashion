  ...
  try {
    ...
    if (db) {
      try {
        ...
        console.warn("DB admin login query error:", err);
      }
    }
    ...
    if ((user.accountStatus || "ACTIVE") !== "ACTIVE") {
      return NextResponse.json(
        { success: false, error: "This administrator account is not active." },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Failed to sign in as admin." },
      { status: 500 }
    );
  }
}
